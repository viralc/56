...
class CrudControllerCommand extends GeneratorCommand
{
    ...
    protected function buildClass($name)
    {
        ...
        $routePath = $this->option('route-group').'.'.$crudNameSingular;
...
$snippet = <<<EOD
        if (\$request->file('{{fieldName}}')) {
            \${{crudNameSingular}}->{{fieldName}} = \$this->upload(\$request->file('{{fieldName}}'), '{{crudNameSingular}}')->getFileName();
        } else {
            \${{crudNameSingular}}->{{fieldName}} = \${{crudNameSingular}}->{{fieldName}};
        }
EOD;

$snippet = str_replace(
  array('{{crudNameSingular}}'), 
  array(trim($crudNameSingular)),
  $snippet
);
        ...
        if ($fields) {
            ...
            foreach ($fieldsArray as $index => $item) {
                ...
                if (trim($itemArray[1]) == 'file' OR trim($itemArray[1]) == 'image') {
...
$whereSnippet .= <<<EOD
        if (\$request->has('{{fieldName}}')) {
            \${{crudName}} = \${{crudName}}->where('{{fieldName}}', 'LIKE', '%'.\$request->get('{{fieldName}}').'%');
        }
EOD;

$whereSnippet  = "\n" . str_replace(
  array('{{crudName}}','{{fieldName}}'), 
  array(trim($crudName),trim($fieldName)), 
  $whereSnippet
);
$whereSnippet .= "\n";

$postSnippet .= <<<EOD
        \${{crudNameSingular}}->{{fieldName}} = \$request->get('{{fieldName}}');
EOD;

$postSnippet  = "\n" . str_replace(
  array('{{crudNameSingular}}','{{fieldName}}'), 
  array(trim($crudNameSingular),trim($fieldName)), 
  $postSnippet
);
$postSnippet .= "\n";
        ...
        return $this->replaceNamespace($stub, $name)
            ...
            ->replacePostSnippet($stub, $postSnippet)
            ->replaceWhiteSpace($stub)
            ->replaceViewroutePath($stub, $routePath)
    ...
    protected function replaceViewroutePath(&$stub, $routePath)
    {
     ...
     * @return $this
     */
    public function replaceWhiteSpace(&$stub)
    {
        //$stub = preg_replace('/\n(\s*\n){2,}/', "\n\n", $stub);
        $stub = preg_replace('/\n(\s*\n)+/', "\n\n", $stub);
        $stub = preg_replace('/\n+/', "\n", $stub);
     ...
     * @return $this
     */
    protected function replacePostSnippet(&$stub, $postSnippet)
    {
        $stub = str_replace('{{postSnippet}}', $postSnippet, $stub);
